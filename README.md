...
- API 配置页同时支持 OpenAI Responses API 和 OpenAI 兼容的 Chat Completions，内置 DeepSeek、阿里云百炼、硅基流动、月之暗面、智谱、百川、腾讯混元、火山方舟等预设，并支持公网 HTTPS 自定义兼容地址。
- 自定义接口地址会拒绝本机、局域网和保留网络；各平台 Key、接口地址和模型 ID 必须配套使用。
